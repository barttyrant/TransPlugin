 ...
 */
class ExtraExtractTask extends AppShell {
 ...
 */
	public function execute() {
        ...
        $defaultParams = array(
            'exclude' => false,
            'path_from' => APP . 'Controller',
            'path_to' => APP . 'Locale',
            'exclude-plugins' => false,
            'ignore-model-validation' => false,
            'validation-domain' => false,
            'merge' => true,
            'paths' => false
        );
        ...
        if (!empty($this->params['paths'])) {
            ...
            $this->_paths = array($this->params['path_from']);
        }        
        ...
        $this->_output = $this->params['path_to'];
...
/**
 * Build the translate template database records out of obtained strings
 *
 * @return void
 */
	protected function _saveToDb() {
		...
		foreach ($this->_translations as $domain => $translations) {
			foreach ($translations as $msgid => $details) {
                ...
                App::uses('Translation', 'Trans.Model');
                                
                $translationModel = new Translation();
                
                if(empty($translationModel) || !is_object($translationModel)){
                    continue;
                }
                
                $hash = sha1($msgid . '-' . $domain);
                $recordExists = $translationModel->find('first', array(
                    'conditions' => array('Translation.msgid_hash' => $hash)
                ));
                ...
                if(empty($recordExists)){
                    ...
                    $saveData = array(
                        ...
                        'locale' => $domain,
                        'msgid_hash' => $hash,
                        'status' => $translationModel::STATUS_UNTRANSLATED,
                        'msgstr' => null
                    );
                    
                    if($translationModel->save(array('Translation' => $saveData))){
                        $saved++;
                    }
                    else{
                        $nsaved++;
        ...
        $this->out('Done');
        if($saved){
            $this->out('Saved ' . $saved . ' new translation lines');
        }
        else{
            $this->out('No new lines saved');
        }        
        if($nsaved){
            $this->out('Unable to save ' . $nsaved . ' translation lines');        
        }
	}
